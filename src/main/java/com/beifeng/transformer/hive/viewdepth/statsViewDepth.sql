...
--7. hql编写(统计用户角度的浏览深度) 查询结果为窄表 注意时间为外部给定
from(
select
pl,
...
pl is not null and
s_time>unix_timestamp('$startDate','yyyy-MM-dd')*1000 and
s_time<unix_timestamp('$endDate','yyyy-MM-dd')*1000
...
--在MySQL中的dimension_kpi表中手动插入(4,'view_depth_of_users')
...
s_time>unix_timestamp('$startDate','yyyy-MM-dd')*1000 and
s_time<unix_timestamp('$endDate','yyyy-MM-dd')*1000
group by pl,from_unixtime(cast(s_time/1000 as bigint), 'yyyy-MM-dd'),u_sid) as tmp
insert overwrite table stats_view_depth_tmp
select pl,day,pv,count(distinct u_sid) cnt
where u_sid is not null
group by pl,day,pv;
...
select platform_convert(pl),date_convert(date),5,sum(pv1),sum(pv2),sum(pv3),sum(pv4),sum(pv5_10),sum(pv10_30)
